...
class TorqueCenterTest(tester.tests.Test):
    ...
    def __init__(self, cancel: tester.tests.CancelToken, devices: DeviceManager = None):
        ...
        """
        logger.debug(
            f"[TorqueCenterTest] Initializing TorqueCenterTest with cancel={cancel}, devices={devices}"
        )
        try:
            super().__init__(
                "Torque Center Test",
            ...
            logger.critical(
                f"[TorqueCenterTest] Exception in __init__: {e}\n{traceback.format_exc()}"
            )
    ...
    def Polyfit(self):
        ...
        """
        polyFit = []
        coeffs = self.getParameter("PolyfitCoeffs", None)
        if coeffs is not None:
            a, b, c = coeffs
            xs = np.linspace(self.xmin, self.xmax, 100)
            ys = a * xs**2 + b * xs + c
            polyFit = [QtCore.QPointF(float(x), float(y)) for x, y in zip(xs, ys)]
        return polyFit
    ...
    def onSettingsModified(self):
        ...
        try:
            ...
            logger.critical(
                f"[TorqueCenterTest] Exception in onSettingsModified: {e}\n{traceback.format_exc()}"
            )
    ...
    def setupReportGenerator(self, reportGenerator):
        ...
        self.torqueCenterChanged.connect(
            lambda value: reportGenerator.onTestResultAdded(
                self.Name,
                "TorqueCenter",
                {"Type": "Text", "Content": f"Torque Center: {value}"},
            )
        )
    ...
    def setupUi(self, parent=None):
        ...
        try:
            ...
            chartViewTorqueCenter = QtCharts.QChartView(
                chartTorqueCenterPlot, self.widgetTestData
            )
            chartViewTorqueCenter.setObjectName("chartViewTorqueCenter")
            chartViewTorqueCenter.setSizePolicy(
            ...
            def updateFitSeries(coeffs):
                ...
                if coeffs is not None:
                    ...
                    xs = np.linspace(self.xmin, self.xmax, 100)
                    ys = a * xs**2 + b * xs + c
                    lineSeriesTorqueFit.append(
                        [QtCore.QPointF(float(x), float(y)) for x, y in zip(xs, ys)]
                    )
    ...
    def onSaveData(self):
        ...
        try:
            ...
            if file.open(QtCore.QIODevice.WriteOnly | QtCore.QIODevice.Text):
                ...
                logger.warning(
                    f"[TorqueCenterTest] Could not open file {dataFilePath} for writing"
                )
        except Exception as e:
            logger.critical(
                f"[TorqueCenterTest] Failed to save torque data: {e}\n{traceback.format_exc()}"
            )
    ...
    def resetTestData(self):
        ...
        try:
            ...
            logger.critical(
                f"[TorqueCenterTest] Exception in resetParameters: {e}\n{traceback.format_exc()}"
            )
    ...
    def setDataDirectory(self, data_directory):
        ...
        try:
            ...
            if not dir_obj.exists():
                logger.warning(
                    f"[TorqueCenterTest] Data directory {self.dataDirectory} does not exist. Creating directory."
                )
                dir_obj.mkpath(".")
            ...
            logger.debug(
                f"[TorqueCenterTest] Data file path set to: {self.dataFilePath}"
            )
        except Exception as e:
            logger.critical(
                f"[TorqueCenterTest] Exception in setDataDirectory: {e}\n{traceback.format_exc()}"
            )
    ...
    def setup(self):
        ...
        """
        logger.debug(
            f"[TorqueCenterTest] Setting up TorqueCenterTest for serial: {self.SerialNumber}"
        )
        try:
            super().setup()
            ...
            logger.critical(
                f"[TorqueCenterTest] Exception in setup: {e}\n{traceback.format_exc()}"
            )
    ...
    def run(self):
        ...
        """
        logger.info(
            f"[TorqueCenterTest] Running TorqueCenterTest for serial: {self.SerialNumber}"
        )
        try:
            super().run()
            mso = self.devices.MSO5000
            ...
            logger.info(
                f"[TorqueCenterTest] Starting data collection loop: sleepDelay={sleepDelay}"
            )
            ...
            for i, (offset, offset_scaled) in enumerate(offsets):
                ...
                for _ in range(0, sleepDelay, 100):
                    ...
                    msleep(100)
                try:
                    rms = mso.get_measure_item(Measurement.VoltageRms, Source.Channel2)
                    logger.info(
            ...
            logger.critical(
                f"[TorqueCenterTest] Exception in run: {e}\n{traceback.format_exc()}"
            )
    ...
    def analyzeResults(self) -> bool:
        ...
        """
        logger.info(
            f"[TorqueCenterTest] Analyzing results for serial: {self.SerialNumber}"
        )
        try:
            ...
            if data:
                ...
                logger.critical(
                    "[TorqueCenterTest] No torque data available for analysis"
                )
            ...
            logger.critical(
                f"[TorqueCenterTest] Exception in analyzeResults: {e}\n{traceback.format_exc()}"
            )
            result = False
    ...
    def teardown(self):
        ...
        try:
            ...
            logger.critical(
                f"[TorqueCenterTest] Exception in teardown: {e}\n{traceback.format_exc()}"
            )
